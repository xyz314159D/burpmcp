...
class AutoApproveTargetsPanel(private val config: McpConfig) : JPanel() {
    ...
    private fun buildPanel() {
        ...
        val descLabel = JLabel("Specify hosts or URL prefixes that can be accessed without approval.").apply {
            alignmentX = LEFT_ALIGNMENT
            font = Design.Typography.bodyMedium
            foreground = Design.Colors.onSurfaceVariant
            border = BorderFactory.createEmptyBorder(0, 0, Design.Spacing.SM, 0)
        }
        val examplesLabel = JLabel("Examples: example.com, localhost:8080, *.api.com, https://api.example.com/v1").apply {
            alignmentX = LEFT_ALIGNMENT
        ...
        val addButton = Design.createFilledButton("Add").apply {
            addActionListener {
                val input = Dialogs.showInputDialog(
                    findBurpFrame(),
                    "Enter target (hostname, hostname:port, wildcard, or URL prefix):\nExamples: example.com, localhost:8080, *.api.com, https://api.example.com/v1"
                )
                ...
                if (!input.isNullOrBlank()) {
                    ...
                    if (TargetValidation.isValidTarget(trimmed)) {
                        ...
                        Dialogs.showMessageDialog(
                            findBurpFrame(),
                            "Invalid target format. Use hostname, IP address, hostname:port, wildcard (*.domain), or URL prefix.",
                            ERROR_MESSAGE
                        )
                    }
                }
