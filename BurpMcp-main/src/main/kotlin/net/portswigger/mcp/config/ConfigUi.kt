...
class ConfigUi(private val config: McpConfig, private val providers: List<Provider>) {
            ...
            when (state) {
                ...
                is ServerState.Failed -> {
                    ...
                    Dialogs.showMessageDialog(
                        panel, "Failed to start BurpMcp: $friendlyMessage", ERROR_MESSAGE
                    )
                }
            }
    ...
    private fun buildUi() {
        ...
        val headerBox = createVerticalBox().apply {
            add(JLabel("BurpMcp").apply {
                font = Design.Typography.headlineMedium
                foreground = Design.Colors.onSurface
                alignmentX = CENTER_ALIGNMENT
            })
            add(createVerticalStrut(Design.Spacing.MD))
            add(JLabel("BurpMcp exposes Burp workflows to AI clients for web pentesting.").apply {
