...
class HttpRequestSecurityTest {
    ...
    @Test
    fun `checkHttpRequestPermission should allow approved url prefix only for matching path and scheme`() {
        config.addAutoApproveTarget("https://example.com/api/v1")

        coEvery { mockApprovalHandler.requestApproval("example.com", 443, config, any(), any()) } returns false
        coEvery { mockApprovalHandler.requestApproval("example.com", 80, config, any(), any()) } returns false
